// generateDaemons.ts
export function generateDaemons(bufferLength: number, daemonCount: number): { id: number, name: string, sequence: string[] }[] {
  ...
  const daemons: { id: number, name: string, sequence: string[] }[] = [];
  ...
  for (let i = 0; i < daemonCount; i++) {
    ...
    daemons.push({ id: i + 1, name: `DATAMINE_V${i + 1}`, sequence });
  }
